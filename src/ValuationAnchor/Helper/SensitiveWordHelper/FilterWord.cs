...
namespace SensitivewordApi.Helper.SensitiveWordHelper
{
    ...
    public class FilterWord
    {
        ...
        //检查源
        public FilterWord(string dictionarys)
        {
            this.dictionarys = dictionarys;
        }

        private string dictionarys = string.Empty;
        ...
        private bool Check(string blackWord)
        {
            ...
            for (int i = 0; i < blackWord.Length; i++)
            {
                //特殊字符偏移游标
                int offset = 0;
                if (nextCursor >= sourctText.Length)//边界值
                {
                    found = false;
                    break;
                }
                else
                {
                    ...
                    if (nextCursor + offset >= sourctText.Length)//边界值
                    {
                        found = false;
                        break;
                    }
                ...
                for (int i = 0; i < SourctText.Length; i++)
                {
                    ...
                    if (group != null)
                    {
                        for (int z = 0; z < group.Count(); z++)
                        {
                            ...
                            if (word.Length == 0 || Check(word))//word.Length == 0 || 
                            {
